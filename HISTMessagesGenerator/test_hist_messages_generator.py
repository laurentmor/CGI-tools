...
class TestHISTMessagesGenerator(unittest.TestCase):
    ...
    def test_build_dictionary_unique(self, mock_resolve):
        ...
        self.logger.info(
            "TEST: test_build_dictionary_unique - Should build dictionary with unique instruments only"
        )
    ...
    def test_duplicate_instrument_keeps_first(self, mock_resolve):
        ...
        self.logger.info(
            "TEST: test_duplicate_instrument_keeps_first - Duplicate instrument IDs keep first occurrence"
        )
    ...
    def test_ignore_null_instrument(self, mock_resolve):
        ...
        xml = self.build_xml([{"TYPE_": "A", "CUSTOMER_PARTY_TYPE": "P1"}])
...
<ROWS>
    <ROW>
        ...
        try:
            ...
            with open(tmp_file.name, "r", encoding="utf-8") as f:
                self.generator.validate_columns_exist(
                    f.name, ["INSTRUMENT_ID", "TYPE_", "CUSTOMER_PARTY_TYPE"]
                )
        finally:
    ...
    def test_validate_columns_missing(self):
        ...
        self.logger.info(
            "TEST: test_validate_columns_missing - Missing required columns should raise ValueError"
        )

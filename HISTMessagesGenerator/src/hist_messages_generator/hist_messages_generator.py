...
class HISTMessagesGenerator:
    ...
    # MAIN EXECUTION
    # ==========================
    @log_exceptions({FileNotFoundError: "Input file not found", ET.ParseError: "Error parsing XML"}, raise_exception=True, logger=lambda self: HISTMessagesGenerator.logger)
    def run(self):
        start = time.time()
...
select * from outgoing_intrfc_e where A_INTRFC_EVENT_TY= 'HIST' and A_INSTRUMENT in ( select UOID from instrument where instrument_id in ( """
        ...
        HISTMessagesGenerator.logger.info(f"*** Generating Inserts {self.customer} - {effective_count} Messages ***")
...
'{self.bank}','S',SYSDATE);\n\n"""
        ...
        HISTMessagesGenerator.logger.info(f"Total execution time: {time.time() - start:.2f} seconds")
    ...
    # VALIDATION METHODS
    # ==========================
    @log_exceptions({Exception: "Error occurred while getting row count"}, raise_exception=True, logger=lambda self: HISTMessagesGenerator.logger)
    def get_row_count(self):
        return sum(1 for _, elem in ET.iterparse(self.input_file) if elem.tag == "ROW")

    @log_exceptions({Exception: "Error occurred while validating XML structure"}, raise_exception=True, logger=lambda self: HISTMessagesGenerator.logger)
...
def main():
    ...
    parser.add_argument('--log-file', type=str, default='HISTMessagesGenerator.log', help='Log file path.')
